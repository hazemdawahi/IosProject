const Lvl3 = require('../models/lvl3')
...
const store = (req, res, next) => {
    ...
    lvl3.iduser = useriden;
    lvl3.nbrstore= t;
    lvl3.save()
   .then(response => 
    {
        res.json({
            message:'Lvl3 Added Successfully'
        })
...
const update = (req, res, next) => {
    ...
    Lvl3.findByIdAndUpdate(lvl3ID, {$set: updatedData})
    .then(() => {
        res.json({
            message:'Lvl3 updated successfully!'
        })
    })
.catch(error => {
    res.json({
...
const destroy =(req, res, next) =>{
    let lvl3ID = req.body.lvlID
    Lvl3.findByIdAndRemove(lvl3ID)
    .then(() => {
        res.json({
            message:'Lvl3 deleted successfully!'
        })
    })
    .catch (error => {
        res.json({
...
var Progress=response.progress
     ...
     if ((responseNbr-array.length!=0)&&(Progress!=4))
     {
 res.json({
     question:Q1
 })
