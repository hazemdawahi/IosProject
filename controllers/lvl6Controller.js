const Lvl6 = require('../models/lvl6')
...
const store = (req, res, next) => {
    ...
    lvl6.iduser = useriden;
    lvl6.nbrstore= t;
    lvl6.save()
   .then(response => 
    {
        res.json({
            message:'Lvl6 Added Successfully'
        })
...
const update = (req, res, next) => {
    ...
    Lvl6.findByIdAndUpdate(lvl6ID, {$set: updatedData})
    .then(() => {
        res.json({
            message:'Lvl6 updated successfully!'
        })
    })
.catch(error => {
    res.json({
...
const destroy =(req, res, next) =>{
    let lvl6ID = req.body.lvlID
    Lvl6.findByIdAndRemove(lvl6ID)
    .then(() => {
        res.json({
            message:'Lvl6 deleted successfully!'
        })
    })
    .catch (error => {
        res.json({
...
var Progress=response.progress
     ...
     if ((responseNbr-array.length!=0)&&(Progress!=7))
     {
 res.json({
     question:Q1
 })
