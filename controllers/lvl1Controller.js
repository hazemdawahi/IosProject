const Lvl1 = require('../models/lvl1')
...
const store = (req, res, next) => {
    ...
    lvl1.iduser = useriden;
    lvl1.nbrstore= t;
    lvl1.save()
   .then(response => 
    {
        res.json({
            message:'Lvl1 Added Successfully'
        })
...
const update = (req, res, next) => {
    ...
    Lvl1.findByIdAndUpdate(lvl1ID, {$set: updatedData})
    .then(() => {
        res.json({
            message:'Lvl1 updated successfully!'
        })
    })
.catch(error => {
    res.json({
...
const destroy =(req, res, next) =>{
    let lvl1ID = req.body.lvlID
    Lvl1.findByIdAndRemove(lvl1ID)
    .then(() => {
        res.json({
            message:'Lvl1 deleted successfully!'
        })
    })
    .catch (error => {
        res.json({
...
var Progress=response.progress
     ...
     if ((responseNbr-array.length!=0)&&(Progress!=2))
     {
 res.json({
     question:Q1
 })
