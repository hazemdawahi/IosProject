const Lvl8 = require('../models/lvl8')
...
const store = (req, res, next) => {
    ...
    lvl8.iduser = useriden;
    lvl8.nbrstore= t;
    lvl8.save()
   .then(response => 
    {
        res.json({
            message:'Lvl8 Added Successfully'
        })
...
const update = (req, res, next) => {
    ...
    Lvl8.findByIdAndUpdate(lvl8ID, {$set: updatedData})
    .then(() => {
        res.json({
            message:'Lvl8 updated successfully!'
        })
    })
.catch(error => {
    res.json({
...
const destroy =(req, res, next) =>{
    let lvl8ID = req.body.lvlID
    Lvl8.findByIdAndRemove(lvl8ID)
    .then(() => {
        res.json({
            message:'Lvl8 deleted successfully!'
        })
    })
    .catch (error => {
        res.json({
...
var Progress=response.progress
     ...
     if ((responseNbr-array.length!=0)&&(Progress!=9))
     {
 res.json({
     question:Q1
 })
