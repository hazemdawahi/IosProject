const Lvl7 = require('../models/lvl7')
...
const store = (req, res, next) => {
    ...
    lvl7.iduser = useriden;
    lvl7.nbrstore= t;
    lvl7.save()
   .then(response => 
    {
        res.json({
            message:'Lvl7 Added Successfully'
        })
...
const update = (req, res, next) => {
    ...
    Lvl7.findByIdAndUpdate(lvl7ID, {$set: updatedData})
    .then(() => {
        res.json({
            message:'Lvl7 updated successfully!'
        })
    })
.catch(error => {
    res.json({
...
const destroy =(req, res, next) =>{
    let lvl7ID = req.body.lvlID
    Lvl7.findByIdAndRemove(lvl7ID)
    .then(() => {
        res.json({
            message:'Lvl7 deleted successfully!'
        })
    })
    .catch (error => {
        res.json({
...
var Progress=response.progress
     ...
     if ((responseNbr-array.length!=0)&&(Progress!=8))
     {
 res.json({
     question:Q1
 })
