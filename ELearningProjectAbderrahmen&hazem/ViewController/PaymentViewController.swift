        ...
        let request = BTPayPalRequest(amount: "300")
            ...
            payPalDriver.requestOneTimePayment(request) { (tokenizedPayPalAccount, error) in
                if let tokenizedPayPalAccount = tokenizedPayPalAccount {
                    ...
                    UserService.shareinstance.changevie1000()
                    self.performSegue(withIdentifier: "segux", sender: nil)
                    self.presentingViewController?.presentingViewController?.dismiss(animated: true, completion: nil)
        ...
        let request = BTPayPalRequest(amount: "30")
            ...
            payPalDriver.requestOneTimePayment(request) { (tokenizedPayPalAccount, error) in
                if let tokenizedPayPalAccount = tokenizedPayPalAccount {
                    ...
                    // Access additional information
                    debugPrint("Payment successfull")
                    UserService.shareinstance.changevie100()
                    self.performSegue(withIdentifier: "segux", sender: nil)
                    self.presentingViewController?.presentingViewController?.dismiss(animated: true, completion: nil)
        ...
        let request = BTPayPalRequest(amount: "3")
            ...
            payPalDriver.requestOneTimePayment(request) { (tokenizedPayPalAccount, error) in
                if let tokenizedPayPalAccount = tokenizedPayPalAccount {
                    ...
                    // Access additional information
                    debugPrint("Payment successfull")
                    UserService.shareinstance.changevie10()
                    self.performSegue(withIdentifier: "segux", sender: nil)
                    self.presentingViewController?.presentingViewController?.dismiss(animated: true, completion: nil)
